...
- [x] **H1.** Add database backend for persistence (completed 2026-02-12) — SQLAlchemy ORM prototype with SQLite, DB-backed CRUD with file fallback, backfill migrations. See [docs/H1_DATABASE_BACKEND.md](docs/H1_DATABASE_BACKEND.md)
- [x] **H1.1.** Containerized data services: PostgreSQL + Minio (completed 2026-02-14) — PostgreSQL (own container) replaces embedded SQLite, Minio S3 object store replaces shared Docker volume. 4-container architecture with clean separation. 37 new tests, 288 total passing. See [docs/H1_DATABASE_BACKEND.md](docs/H1_DATABASE_BACKEND.md) section "Implementation Summary (H1.1)"
...
2. **H1** - Database backend (data persistence) — DONE
3. **H1.1** - Containerized data services (clean separation) — DONE
4. **H9** - Rate limiting (DOS protection)
5. **M16-M17** - Structured logging & rotation — DONE
6. **H13** - CI/CD pipeline
...
- Probe Tag Filtering (OWASP LLM Top 10)
- Missing Generator Types (Groq, Mistral, Azure, Bedrock)
...
- Buff Options: Include Original Prompt
- Increase Max Generations Limit
